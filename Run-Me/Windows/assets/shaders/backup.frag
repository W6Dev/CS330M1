...
vec3 roomLight()
{
    vec3 sampledTexture = texture(basicTexture, texCoord);  // Sample the texture
    vec3 objectColor = vertexColor.rgb * sampledTexture;  // Modulate with vertexColor

    // Ambient lighting
    float LightBrightness = .9;
    vec3 ambient = LightBrightness * vec3(1.0f, 1.0f, 1.00f) * objectColor;
